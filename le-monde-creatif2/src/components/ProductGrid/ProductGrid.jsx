...
const ProductGrid = ({setChangeVue}) => {
  ...
  const getLast6Product = async()=>{
    try{
      const response= await axios.get(PORT+"articles/lastArticle")
      if(response.status==200){
        setHomeArt(response.data)
      }
    }catch(e){
      console.log('error'+e);
  ...
  return (
    <section style={productSectionStyle}>
      <div style={sectionHeaderStyle}>
        <h2 style={sectionTitleStyle}>Nos Dernier Arictle</h2>
